#!/usr/bin/env bash
# SmoothQuant W8A8 — allenai/Llama-3.1-Tulu-3-8B → Llama-3.1-Tulu-3-8B-SmoothQuant-W8A8
...
OUTPUT_NAME="Llama-3.1-Tulu-3-8B-SmoothQuant-W8A8"
OUTPUT_DIR="$RUN_DIR/output"
mkdir -p "$OUTPUT_DIR"

# shellcheck disable=SC2086
uv run "$TECH_DIR/src/smoothquant.py" \
  --hf-model "$HF_MODEL" \
  --output-dir "$OUTPUT_DIR" \
  --output-model-name "$OUTPUT_NAME" \
  --show-inference-instruction \
  --alpha 0.5 --nsamples 128 --seqlen 512 --calib-dataset pileval \
  ${EXTRA_ARGS:-} "$@"
