#!/usr/bin/env bash
# FP8 E4M3 — allenai/Llama-3.1-Tulu-3-8B → Llama-3.1-Tulu-3-8B-FP8-E4M3
...
OUTPUT_NAME="Llama-3.1-Tulu-3-8B-FP8-E4M3"
...
uv run "$TECH_DIR/src/fp8.py" \
  --hf-model "$HF_MODEL" \
  --output-dir "$OUTPUT_DIR" \
  --output-model-name "$OUTPUT_NAME" \
  --show-inference-instruction \
  --fp8-format e4m3n --activation-scheme dynamic --weight-block-size 128 \
  ${EXTRA_ARGS:-} "$@"
