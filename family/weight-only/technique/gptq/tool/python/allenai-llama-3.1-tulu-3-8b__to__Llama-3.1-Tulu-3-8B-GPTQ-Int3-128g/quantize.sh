#!/usr/bin/env bash
# GPTQ INT3 — allenai/Llama-3.1-Tulu-3-8B → Llama-3.1-Tulu-3-8B-GPTQ-Int3-128g
# 어느 디렉토리에서 실행해도 동일 결과 (이 파일 기준 절대경로).
# 의존성은 src/*.py 상단 PEP 723 블록이 선언 (venv/requirements 불필요).
# NOTE: 이 technique은 .python-version 3.11 override 적용 (auto-gptq 3.12 미지원).
...
OUTPUT_NAME="Llama-3.1-Tulu-3-8B-GPTQ-Int3-128g"
...
uv run "$TECH_DIR/src/gptq.py" \
  ...
  --bits 3 --group-size 128 --sym --desc-act \
  --dataset wikitext2 --nsamples 256 --seqlen 2048 \
  ${EXTRA_ARGS:-} "$@"
