#!/usr/bin/env bash
# SpinQuant INT4 — allenai/Llama-3.1-Tulu-3-8B → Llama-3.1-Tulu-3-8B-SpinQuant-Int4-128g
...
OUTPUT_NAME="Llama-3.1-Tulu-3-8B-SpinQuant-Int4-128g"
OUTPUT_DIR="$RUN_DIR/output"
mkdir -p "$OUTPUT_DIR"

# shellcheck disable=SC2086
uv run "$TECH_DIR/src/spin-quant.py" \
  --hf-model "$HF_MODEL" \
  --output-dir "$OUTPUT_DIR" \
  --output-model-name "$OUTPUT_NAME" \
  --show-inference-instruction \
  --bits 4 --group-size 128 --rotation hadamard \
  ${EXTRA_ARGS:-} "$@"
