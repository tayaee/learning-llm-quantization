#!/usr/bin/env bash
# SpinQuant W4A8 (Meta 공식 학습) — allenai/Llama-3.1-Tulu-3-8B → Llama-3.1-Tulu-3-8B-SpinQuant-W4A8-meta
# 어느 디렉토리에서 실행해도 동일 결과 (이 파일 기준 절대경로).
# 의존성은 src/*.py 상단 PEP 723 블록이 선언 (venv/requirements 불필요).
# NOTE: 공식 저장소는 tool/meta/third_party/SpinQuant에 체크아웃 (git 추적 제외).
# 인프라: INFRA=<infra-slug> ./run.sh  (기본 dgx-spark-1x)
set -euo pipefail
...
OUTPUT_NAME="Llama-3.1-Tulu-3-8B-SpinQuant-W4A8-meta"
OUTPUT_DIR="$RUN_DIR/output"
mkdir -p "$OUTPUT_DIR"

# shellcheck disable=SC2086
uv run "$TECH_DIR/src/spinquant-meta.py" \
  --hf-model "$HF_MODEL" \
  --output-dir "$OUTPUT_DIR" \
  --output-model-name "$OUTPUT_NAME" \
  --show-inference-instruction \
  --w-bits 4 --a-bits 8 --kv-bits 8 --bits 4 --group-size 128 \
  ${EXTRA_ARGS:-} "$@"
