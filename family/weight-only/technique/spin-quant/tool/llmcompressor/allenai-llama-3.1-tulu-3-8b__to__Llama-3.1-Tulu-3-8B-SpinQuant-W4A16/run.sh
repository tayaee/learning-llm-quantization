#!/usr/bin/env bash
# SpinQuant W4A16 (llmcompressor, datafree) — allenai/Llama-3.1-Tulu-3-8B → Llama-3.1-Tulu-3-8B-SpinQuant-W4A16
...
OUTPUT_NAME="Llama-3.1-Tulu-3-8B-SpinQuant-W4A16"
OUTPUT_DIR="$RUN_DIR/output"
mkdir -p "$OUTPUT_DIR"

# shellcheck disable=SC2086
uv run "$TECH_DIR/src/spinquant-llmcompressor.py" \
  --hf-model "$HF_MODEL" \
  --output-dir "$OUTPUT_DIR" \
  --output-model-name "$OUTPUT_NAME" \
  --show-inference-instruction \
  --rotations R1,R2,R4 --transform-block-size 128 --scheme W4A16 \
  ${EXTRA_ARGS:-} "$@"
