#!/usr/bin/env bash
# AQLM 2bit — allenai/Llama-3.1-Tulu-3-8B → Llama-3.1-Tulu-3-8B-AQLM-2bit-1x16
...
OUTPUT_NAME="Llama-3.1-Tulu-3-8B-AQLM-2bit-1x16"
...
uv run "$TECH_DIR/src/aqlm.py" \
  --hf-model "$HF_MODEL" \
  --output-dir "$OUTPUT_DIR" \
  --output-model-name "$OUTPUT_NAME" \
  --show-inference-instruction \
  --bits 2 --num-codebooks 1 --nbits-per-codebook 16 \
  --in-group-size 8 --nsamples 128 --seqlen 4096 \
  ${EXTRA_ARGS:-} "$@"
