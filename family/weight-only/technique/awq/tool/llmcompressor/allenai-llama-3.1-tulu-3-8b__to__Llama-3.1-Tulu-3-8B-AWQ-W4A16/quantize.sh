#!/usr/bin/env bash
# AWQ W4A16 (llmcompressor) — allenai/Llama-3.1-Tulu-3-8B → Llama-3.1-Tulu-3-8B-AWQ-W4A16
...
OUTPUT_NAME="Llama-3.1-Tulu-3-8B-AWQ-W4A16"
...
uv run "$TECH_DIR/src/awq-llmcompressor.py" \
  --hf-model "$HF_MODEL" \
  --output-dir "$OUTPUT_DIR" \
  --output-model-name "$OUTPUT_NAME" \
  --show-inference-instruction \
  --scheme W4A16_ASYM --nsamples 512 --seqlen 2048 \
  ${EXTRA_ARGS:-} "$@"
