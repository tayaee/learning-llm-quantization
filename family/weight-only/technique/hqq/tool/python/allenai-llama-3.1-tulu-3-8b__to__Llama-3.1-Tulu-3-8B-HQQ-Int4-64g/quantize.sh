#!/usr/bin/env bash
# HQQ INT4 — allenai/Llama-3.1-Tulu-3-8B → Llama-3.1-Tulu-3-8B-HQQ-Int4-64g
...
OUTPUT_NAME="Llama-3.1-Tulu-3-8B-HQQ-Int4-64g"
...
uv run "$TECH_DIR/src/hqq.py" \
  --hf-model "$HF_MODEL" \
  --output-dir "$OUTPUT_DIR" \
  --output-model-name "$OUTPUT_NAME" \
  --show-inference-instruction \
  --bits 4 --group-size 64 --axis 0 --optimize \
  ${EXTRA_ARGS:-} "$@"
