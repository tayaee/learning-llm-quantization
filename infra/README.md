...
`quantize.sh`는 `INFRA=<infra-slug>` 환경변수로 이 디렉토리 아래 `env.sh`를 소싱한다.
비밀(`HF_TOKEN` 등)은 env.sh에 절대 넣지 않고, 호출자가 미리 export한다.

```bash
./quantize.sh                                            # 기본 하드웨어 dgx-spark-1x
INFRA=dgx-spark-2x SPARK_NODE=1 ./quantize.sh
INFRA=runpod-h100-1x HF_TOKEN=... ./quantize.sh
```
...
양자화 작업 자체는 단일 GPU 작업이므로, 2x는 **두 quantize.sh를 각 본체에서**
나눠 실행하는 용도다 (`SPARK_NODE=0/1`로 매트릭스 분할).
듀얼노드 TP 서빙은 별도 Ray 구성이 필요하며 이 레포 범위가 아니다.

## Python 버전

루트 `.python-version`(3.12)이 기본. `quantize.sh`는 실행 전 technique 디렉터리로
